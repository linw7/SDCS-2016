...
void heap_modify(int cur, int length)
{
	...
	if((rightchild < length) && (items[leftchild].divide >= items[rightchild].divide))
		max = leftchild;
	else if((leftchild < length) && (items[rightchild].divide >= items[leftchild].divide))
		max = rightchild;
	if(rightchild == length)
		max = leftchild;

	if(max != cur){
		if(items[max].divide > items[cur].divide){
			swap(cur, max);
