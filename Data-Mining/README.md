...
这种模式在多个复制了数据的站点中选择一个站点的副本作为主副本。在有事务请求需要封锁Q时，要对主副本所在的站点请求封锁。请求的响应会推迟到该请求能被满足为止。
...
1. 关系OLAP服务器：
	...
	中间服务器，介于关系的后端服务器和客户前端工具之间。使用关系的或扩充关系的DBMS存储并管理数据仓库数据，而OLAP中间件支持其余部分。
2. 多维OLAP服务器：

	通过基于数组的多维存储引擎支持数据的多维视图，将多维视图直接映射到数据立方体数组结构。
	
3. 混合OLAP服务器：

	结合ROLAP和MOLAP技术，得益于ROLAP较大的可伸缩性和MOLAP的快速计算
...
- 当近邻计算需要计算所有点对的邻近度时（高维数据常是如此），DBSCAN的开销可能很大。
